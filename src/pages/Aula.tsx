...
const Aula = () => {
  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      <main className={`w-full max-w-5xl mx-auto px-2 sm:px-6 ${aula.titulo ? "pt-6 md:pt-12" : "pt-4 md:pt-6"} pb-10 md:pb-16`}>
        {aula.titulo && (
          <h1 className="font-heading text-2xl md:text-4xl font-bold text-primary text-center mb-6 px-2">
            {aula.titulo}
          </h1>
        )}

        <article className="space-y-6">
          {/* Player — destaque maior no mobile/tablet para facilitar tela cheia */}
          ...
          {/* Contador no rodapé do vídeo */}
          {startTs && now < startTs && <Countdown target={startTs} />}

          {aula.descricao && (
            <p className="font-body text-base md:text-lg text-foreground/90 whitespace-pre-line leading-relaxed px-2 sm:px-0">
              {aula.descricao}
            </p>
          )}
